...
public abstract class AbstractCrudDao<T extends Identifiable> implements SimpleCrudDao<T> {
    ...
    /**
     * Позволяет методу {@link #update(T)} обнавлять поля старой сущности полями
     * новой
     * <p>
     * Пример:
     * 
     ...
     * 
     * @param namedQuery
     *            именнованный запрос для определенной сущности
     * @return
     */
     ...
     * 
     * @param taker
     *            или consumer: потребитель
     * @param giver
     *            или producer: издатель
     * @param Fields
     *            массив полей в формате: "<b>F</b>ield", а не "<b>f</b>ield"
     */
     ...
     *            набор children
     * @param dao
     *            экземпляр {@link SimpleCrudDao} типизированный типом children
     * @param childMethod
     *            сеттер на поле с типом parent в классе child
     * @return возвращает обновленный набор children
     */
    @SuppressWarnings("unchecked")
    protected <E extends Identifiable> Set<E> updateChild(T type, Set<E> detachedChild,
            SimpleCrudDao<E> dao, String childMethod) {
