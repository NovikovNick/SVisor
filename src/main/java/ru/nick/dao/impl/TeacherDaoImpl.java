...
/**
 * Класс-наследник {@link AbstractCrudDao}. Отвечает за преподавателя и
 * реализует {@link EntityDao}
 * 
 * @author NovikovNick
 *
 */
...
public class TeacherDaoImpl extends AbstractCrudDao<Teacher> implements EntityDao<Teacher> {
    ...
    public Teacher update(Teacher detached) {
        ...
        fieldUpdateInCicle(teacher, detached, "FstName", "SndName", "Surname", "Degree", "Title",
                "Login", "Password", "Inn", "PensionInsurance");

        teacher.setDisciplines(updateChild(teacher, detached.getDisciplines(), discDao,
                "getTeachers"));
